...
let Add = () => {
  if (addItem.value.length > 0) {
    ...
    console.log(index);
    if (index == -1) {
      productList.push({ item: addItem.value, qty: addQty.value, done: false});
    } else {
...
let pendingItem = computed(() => productList.filter(item => !item.done))

let totalPending = computed(() => pendingItem.value.reduce((total,current)=>total+current.qty,0))
...
let doneItem = computed(() => productList.filter(item => item.done))
    ...
    <div class="buying-list-container">
      ...
      <div id="buying-list" style="line-height: 180%">
        <div v-for="(item, index) in productList" :key="index">
          <input type="checkbox" :value="item" v-model="item.done" :id="index"/>
          <label :for="index" :style=" item.done ? doneText : -1 ">
            {{ item.item }}, {{ item.qty }} unit : 
          </label>
          <span v-html="!item.done ? imgPending : imgDone"></span>
        </div>
        {{productList}} <br>
        {{pendingItem}} <br>
        {{doneItem}}
